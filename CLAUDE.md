...
**命名原則**（未來擴充時遵守）：
- 互動網站 → 根目錄或 kebab-case 子目錄（例：`fractions-drag/`）
- 紙本練習 → `worksheets/<題型>/`
...
- 測試：Vitest（53 tests passing）

## 檔案結構
- `js/division.js` — 核心除法邏輯（純函式）
- `js/app.js` — UI 渲染 + 互動 + 遊戲化（狀態機、grid 渲染、數字鍵盤、延遲提示、streak、星星、累計星星、里程碑徽章、魔王挑戰）
...
## 核心函式（js/division.js）
- `generateProblem(digitCount=3, divisorMin=2)` — 隨機產生題目（支援 3-6 位數，可指定最小除數）
